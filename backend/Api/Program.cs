...
// Fix Sp_Maintask to include @Catelogid and missing columns like Duration
...
app.MapGet("/api/debug/users", async (SqlConnection connection) => 
{
    var users = new List<object>();
    await connection.OpenAsync();
    using (var cmd = new SqlCommand("SELECT Userid, Email, Role, Catelogid FROM Tbl_Registration", connection))
    using (var reader = await cmd.ExecuteReaderAsync())
    {
        while (await reader.ReadAsync())
        {
            var r = reader["Role"]?.ToString() ?? "";
            users.Add(new { 
                Userid = reader["Userid"]?.ToString(), 
                Email = reader["Email"]?.ToString(), 
                Role = r,
                RoleLength = r.Length,
                Catelogid = reader["Catelogid"]?.ToString()
            });
        }
    }
