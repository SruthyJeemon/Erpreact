...
const Dashboard = () => {
        ...
        for (const module of accessibleModules) {
          ...
          let submenus = accessibleSubModules.map(sm => {
            ...
            if (normalizedMod === 'approvals' && (normalizedSM === 'product' || normalizedSM === 'products')) {
              ...
              foundMapping = true;
            } else if (normalizedMod === 'approvals' && (normalizedSM.includes('purchase') || normalizedSM.includes('bill'))) {
              adminSectionId = 'purchase-approval-hub';
              foundMapping = true;
            }
  ...
  const drawerContent = (
    <Box sx={{ display: 'flex', flexDirection: 'column', height: '100%', bgcolor: '#052859', color: '#ffffff' }}>
      ...
      >
        <MenuItem onClick={() => { handleUserMenuClose(); setActiveSection('admin-profile'); navigate('/admin-profile'); }}>
          <AccountCircleIcon sx={{ fontSize: 18, opacity: 0.7 }} />
          Profile
        </MenuItem>
        <MenuItem onClick={() => { handleUserMenuClose(); setActiveSection('admin-security'); navigate('/admin-security'); }}>
  ...
  return (
    <Box sx={{ display: 'flex', width: '100%', minHeight: '100vh', boxSizing: 'border-box', overflowX: 'hidden' }}>
        ...
        <Toolbar sx={{ justifyContent: 'space-between', px: { xs: 1, sm: 3 }, minHeight: { xs: '60px', sm: '70px' } }}>
          ...
          <Stack direction="row" spacing={{ xs: 0.5, sm: 1 }} alignItems="center">
            ...
            <Popover
              ...
              PaperProps={{ sx: { p: 2, width: 280, borderRadius: 3, boxShadow: '0 10px 40px rgba(0,0,0,0.2)' } }}
            >
      ...
      >
        ...
        {activeSection === 'sales-quote' && <SalesQuoteSection onAddQuote={() => setActiveSection('sales-quote-create')} onEditQuote={(id) => setActiveSection(`sales-quote-create/${id}`)} />}
...
const SalesReturnManagementSection = () => {
  ...
  const getParentConditionName = (sr) => {
    if (sr.parentconditionname || sr.Parentconditionname) {
      return sr.parentconditionname || sr.Parentconditionname;
    }
    const parentId = sr.parentcondition || sr.Parentcondition;
  ...
  const handleSubmit = async (e) => {
    ...
    try {
      const query = editingSalesReturn ? 2 : 1;
  ...
  const handleDelete = async (id) => {
    if (!window.confirm('Are you sure you want to delete this sales return condition?')) return;
    setLoading(true);
    try {
      const response = await fetch(`${API_URL}/api/salesreturn/${id}`, { method: 'DELETE' });
      const data = await response.json();
      if (data.success) {
  ...
  return (
    <Box sx={{ p: 4, bgcolor: '#f8fafc', minHeight: '100%', width: '100%', borderRadius: '20px' }}>
      <Box sx={{ mb: 4 }}>
        ...
        <form onSubmit={handleSubmit}>
          ...
          <DialogActions sx={{ px: 3, py: 3, borderTop: '1px solid #f1f5f9' }}>
            ...
            <Button type="submit" variant="contained" disabled={loading} sx={{ bgcolor: '#3b82f6' }}>
              {loading ? <CircularProgress size={20} color="inherit" sx={{ mr: 1 }} /> : (editingSalesReturn ? 'Update' : 'Create')}
            </Button>
          </DialogActions>
        </form>
      </Dialog>
