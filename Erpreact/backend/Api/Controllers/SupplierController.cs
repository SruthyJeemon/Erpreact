...
namespace Api.Controllers
{
    ...
    public class SupplierController : ControllerBase
    {
        ...
        // GET: api/supplier/details
        // GET: api/supplier/costing-suppliers
        [HttpGet("costing-suppliers")]
        public IActionResult GetCostingSuppliers(string catalogId = "")
        {
            List<object> supplierList = new List<object>();
            ...
            try
            {
                using (var con = new SqlConnection(connectionString))
                {
                    ...
                    string supplierSql = @"SELECT s.Id, s.Supplierdisplayname, s.Openingbalance, s.Currency, s.Typeofsupplier 
                                         FROM Tbl_Supplier s 
                                         LEFT JOIN Tbl_Registration r ON s.Userid = r.Userid 
                                         WHERE (s.Isdelete = 0 OR s.Isdelete IS NULL) 
                                         AND (s.Status = 'Active' OR s.Status IS NULL OR s.Status = '')";
                    ...
                    {
                        supplierSql += " AND r.Catelogid = @CatalogId";
                    }

                    using (var cmd = new SqlCommand(supplierSql, con))
                    {
                        if (!string.IsNullOrEmpty(catalogId))
                        {
                        ...
                        using (var da = new SqlDataAdapter(cmd))
                        {
                            ...
                            foreach (DataRow row in dt.Rows)
                            {
                                var model = new
                                {
                                    ...
                                    supplierdisplayname = row["Supplierdisplayname"]?.ToString() ?? "",
                                    currency = row.Table.Columns.Contains("Currency") ? row["Currency"]?.ToString() : "AED",
                                    typeofsupplier = row.Table.Columns.Contains("Typeofsupplier") ? row["Typeofsupplier"]?.ToString() : ""
                                };
                    ...
                    using (var cmd = new SqlCommand("Sp_billformat", con))
                    {
                        cmd.CommandType = CommandType.StoredProcedure;
                        cmd.Parameters.AddWithValue("@Id", "");
                        ...
                        try
                        {
                            using (var da = new SqlDataAdapter(cmd))
                            {
                                ...
                                if (dt1.Rows.Count > 0)
                                {
                                    string format = dt1.Rows[0][0].ToString();
                                    DateTime currentDate = DateTime.Now;
                                    formattedString = format
                                        .Replace("{YY}", (currentDate.Year % 100).ToString("D2"))
                                        .Replace("{MM}", currentDate.Month.ToString("D2"))
                                        .Replace("{DD}", currentDate.Day.ToString("D2"))
                                        .Replace("{yyyy}", currentDate.Year.ToString("D4"));
                                }
                            }
                        }
                        catch (Exception ex)
                        {
                            // If Sp_billformat fails, we still want the supplier list
                            formattedString = "PI-" + DateTime.Now.ToString("yyyyMMdd") + "-001";
                        }
                    }
                }
                return Ok(new { list = supplierList, formattedString = formattedString });
            }
            catch (Exception ex)
            {
