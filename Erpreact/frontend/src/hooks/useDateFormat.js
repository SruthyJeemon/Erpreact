            ...
            try {
                ...
                if ((result.success || result.Success) && (result.data || result.Data || result.dateFormats || result.DateFormats)) {
                    ...
                    if (data && data.length > 0) {
                        const rawFormat = data[0].dateformat || data[0].Dateformat || 'dd-MM-yyyy';
                        // Extract only the date part (strip time tokens after a space or T)
                        const dateOnlyFormat = rawFormat.split(/[ T]/)[0];
                        setDateFormat(dateOnlyFormat);
                    }
                }
            } catch (error) {
